...
static uint32_t cam_cdm_get_cmd_header_size(unsigned int command)
{
	return CDMCmdHeaderSizes[command];
}

static uint32_t cam_cdm_required_size_dmi(void)
{
	return cam_cdm_get_cmd_header_size(CAM_CDM_CMD_DMI);
}

static uint32_t cam_cdm_required_size_reg_continuous(uint32_t numVals)
{
	if (!numVals) {
...
static uint32_t cam_cdm_required_size_reg_random(uint32_t numRegVals)
{
	return cam_cdm_get_cmd_header_size(CAM_CDM_CMD_REG_RANDOM) +
		(2 * numRegVals);
}

static uint32_t cam_cdm_required_size_indirect(void)
{
	return cam_cdm_get_cmd_header_size(CAM_CDM_CMD_BUFF_INDIRECT);
}

static uint32_t cam_cdm_required_size_genirq(void)
{
	return cam_cdm_get_cmd_header_size(CAM_CDM_CMD_GEN_IRQ);
}

static uint32_t cam_cdm_required_size_wait_event(void)
{
	return cam_cdm_get_cmd_header_size(CAM_CDM_CMD_WAIT_EVENT);
}

static uint32_t cam_cdm_required_size_changebase(void)
{
	return cam_cdm_get_cmd_header_size(CAM_CDM_CMD_CHANGE_BASE);
}

static uint32_t cam_cdm_required_size_comp_wait(void)
{
	return cam_cdm_get_cmd_header_size(CAM_CDM_CMD_COMP_WAIT);
}

static uint32_t cam_cdm_required_size_clear_comp_event(void)
{
	return cam_cdm_get_cmd_header_size(CAM_CDM_CLEAR_COMP_WAIT);
}

static uint32_t cam_cdm_required_size_prefetch_disable(void)
{
	return cam_cdm_get_cmd_header_size(CAM_CDM_WAIT_PREFETCH_DISABLE);
}

static uint32_t cam_cdm_offsetof_dmi_addr(void)
{
	return offsetof(struct cdm_dmi_cmd, addr);
}

static uint32_t cam_cdm_offsetof_indirect_addr(void)
{
	return offsetof(struct cdm_indirect_cmd, addr);
}

static uint32_t *cam_cdm_write_dmi(uint32_t *pCmdBuffer, uint8_t dmiCmd,
...
static uint32_t *cam_cdm_write_regcontinuous(uint32_t *pCmdBuffer, uint32_t reg,
...
static uint32_t *cam_cdm_write_regrandom(uint32_t *pCmdBuffer, uint32_t numRegVals,
	uint32_t *pRegVals)
{
	uint32_t i;
	uint32_t *dst, *src;
...
static uint32_t *cam_cdm_write_indirect(uint32_t *pCmdBuffer, uint32_t indirectBufAddr,
	uint32_t length)
{
...
static void cam_cdm_write_genirq(uint32_t *pCmdBuffer, uint32_t userdata,
...
static uint32_t *cam_cdm_write_wait_event(uint32_t *pcmdbuffer, uint32_t iw,
...
static uint32_t *cam_cdm_write_changebase(uint32_t *pCmdBuffer, uint32_t base)
{
...
static uint32_t *cam_cdm_write_wait_comp_event(
...
static uint32_t *cam_cdm_write_clear_comp_event(
	uint32_t *pCmdBuffer, uint32_t mask1, uint32_t mask2)
{
...
static uint32_t *cam_cdm_write_wait_prefetch_disable(
	uint32_t                   *pCmdBuffer,
	uint32_t                    id,
	uint32_t                    mask1,
	uint32_t                    mask2)
