...
static int _cmm_cbuf_sgt_alloc(struct cmm_buffer *cbuf)
{
	...
	if (!sgt->nents) {
		CAM_ERR(CAM_MEM, "DMA mapping failed");
		rc = -ENOMEM;
		goto err_free_table;
	}
...
static struct sg_table *_cmm_dma_sg_table_new(struct sg_table *sgt)
{
	...
	for_each_sg(sgt->sgl, scl, sgt->orig_nents, i) {
		...
		if (!new_scl && i < sgt->orig_nents - 1) {
			CAM_ERR(CAM_MEM, "Failed to get next sg entry at index %d", i);
			      ...
			      struct dma_buf_attachment *attachment)
{
	...
	if (IS_ERR_OR_NULL(sgt)) {
		rc = IS_ERR(sgt) ? PTR_ERR(sgt) : -ENOMEM;
		goto err_free_att;
	}
...
struct dma_buf *cmm_alloc_buffer(size_t len, unsigned int flags)
{
	...
	if (!exp_info.exp_name) {
		CAM_ERR(CAM_MEM, "Cannot allocate memory for exp_name");
		_cmm_cbuf_destroy(buffer);
		return ERR_PTR(-ENOMEM);
	}
...
int cam_buf_mgr_init(struct platform_device *pdev)
{
	...
	rc = dma_coerce_mask_and_coherent(idev->dev, DMA_BIT_MASK(64));
	if (rc) {
		CAM_ERR(CAM_MEM, "Failed to set DMA mask");
		goto err_free_idev;
	}
